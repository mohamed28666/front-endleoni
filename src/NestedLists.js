...
export default function NestedLists(props) {
  ...
  const render_component = (state, line_name, index, segment, etatcourante, index1) => () => {
    ...
    axios.get('http://192.168.1.28:3333/mh1/line_number/' + state + '/' + segment + '/RT_RATIO/START_TIM/' + line_name).then(resp => { });
    axios.get('http://192.168.1.28:3333/mh2/line_number/' + state + '/' + segment + '/RT_RATIO/START_TIM/' + line_name).then(resp => { });
